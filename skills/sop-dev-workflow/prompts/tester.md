...
- **断言演进授权**：里程碑语义变更时有权更新既有测试断言（注明理由）；禁止为通过弱化断言。
- **执行纪律（防停滞）**：任何命令 120s 无输出即中止记录；产出部分报告后再继续；jsdom 下统一 vi.mock ECharts；后端测试 mock provider/db 禁真实网络。
- **报告防覆盖**：多模块/多里程碑独立命名（qa_final_report_frontend_m3.json 等），coverage 注明统计集合；不覆盖同级既有交付报告。
